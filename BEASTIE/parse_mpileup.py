...
def GATK_ParseMpileup(record,hets,indels,snp,counter,bi,i,pipeup_dict,min_cov,either_cov):
    ...
    if ref != "" and alt != "": #print(list(pipeup_line[4]))
        ...
        if_sv="NA"
        if_snp="NA"
        if totalCount >= int(min_cov) and ref_count >= int(either_cov) and alt_count >= int(either_cov):
            hets+=1
            if length_key == 1:
                if_biallelic="Y"
                bi+=1
...
def Parse_mpileup_allChr(vcfFile_sample,inputGZ,pileup_file,min_cov,either_cov,output):
    ...
    with open(pileup_file, "r") as stream_in:
        ...
        bi = 0
        vcf = VCF(inputGZ)
        vcf.set_samples([vcfFile_sample])  # prefix must be consistent with the sample name in VCF
        for record in vcf:
            counter += 1
    ...
    logging.info('{0} het sites found with total counts above {1} and either allele above {2}'.format(hets,min_cov,either_cov))
