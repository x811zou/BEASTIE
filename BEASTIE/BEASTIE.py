...
def main():
   ...
   parsed_mpileup=pd.read_csv(out+prefix+"_parsed_mpileup.tsv",sep="\t",header=0,index_col=False)
   ...
   start_timestamp = calendar.timegm(time.gmtime())
   cmd="Rscript --vanilla predict_lambda_phasingError.R %s %s %s %s"%(alpha,out,prefix,modelName)
   os.system(cmd)
   stop_timestamp = calendar.timegm(time.gmtime()) 
