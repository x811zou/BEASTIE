...
def main():
    ...
    # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    parsed_mpileup = Parse_mpileup_allChr(
        prefix, vcfgz_file, pileup_file, min_total_cov, out, DEBUG=DEBUG
    )
    # input:  HG00096.pileup
    # output: HG00096_parsed_mpileup.tsv
    parsed_mpileup = pd.read_csv(
        out + prefix + "_parsed_mpileup.tsv", sep="\t", header=0, index_col=False
    )
    # why NA12878 cannot finish this step?
    # print(parsed_mpileup.head())

    # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    print(
        ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> step2: filter input - only keep exonic het SNPs, file for lambda prediction"
    )
    # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    start_timestamp = calendar.timegm(time.gmtime())
    parsed_mpileup_exonicHetSNPs, _ = Intersect_exonicHetSnps(
        parsed_mpileup,
        meta_file,
        prefix,
        out,
    ...
    print(
        "\tTotal time to complete step2 : %d seconds"
    ...
    #  parsed_mpileup_exonicHetSNPs=pd.read_csv(out+str(prefix)+"_parsed_mpileup_exonicHetSnps.tsv",sep="\t",header=0,index_col=False)
    #  print(parsed_mpileup_exonicHetSNPs.head())
    # init_start_timestamp = calendar.timegm(time.gmtime())
    # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    print(
        ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> step3: save data in format for model input"
    )
    # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    start_timestamp = calendar.timegm(time.gmtime())
    ...
    stop_timestamp = calendar.timegm(time.gmtime())
    print(
        "\tTotal time to complete step3 : %d seconds"
        % (stop_timestamp - start_timestamp)
    )
    ...
    print(
        ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> step4: predict phasing error - R"
    )
    ##### 2.use converted format to predict lambda
    start_timestamp = calendar.timegm(time.gmtime())
    cmd = "Rscript --vanilla predict_lambda_phasingError.R %s %s %s %s" % (
        alpha,
        out,
        prefix,
        modelName,
    ...
    stop_timestamp = calendar.timegm(time.gmtime())
    print(
        "\tTotal time to complete step4 : %d seconds"
        % (stop_timestamp - start_timestamp)
    )
    ...
    print(
        ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> step5: update model input with phasing"
    )
    start_timestamp = calendar.timegm(time.gmtime())
    update_model_input_lambda_phasing(out, prefix, "pred_error_GIAB")
    stop_timestamp = calendar.timegm(time.gmtime())
    print(
        "\tTotal time to complete step 5: %f seconds"
        % (stop_timestamp - start_timestamp)
    )
    ...
    # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    print(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> step6: run model")
    # #>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    # print(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> step6.1: run iBEASTIE")
    # ##### 1. use data output from 1 and 2 to run BEASTIE model
    start_timestamp = calendar.timegm(time.gmtime())
    lambdas_file = "/Users/scarlett/Box/Allen_lab/Backup/BEASTIE3/github_example/output/NA12878/NA12878_parsed_mpileup_overlapped_exonicHetSnps_Lambda_0.05_prediced.tsv"
    # #inFile="./output/NA12878_parsed_mpileup_exonicHetSnps_model_input_w_phasingError.txt"
    inFile = "NA12878_parsed_mpileup_exonicHetSnps_model_input.txt"
    in_path = out
    ...
    cmd = "python phasing_stan_wrapper_no_rep_iBEASTIE.py %s %s %s %s %s %s %s %s %s %s %s" % (
        inFile,
        sigma,
        99,
        model,
    ...
    stop_timestamp = calendar.timegm(time.gmtime())
    print(
        "\tTotal time to complete step 6: %d seconds"
        % (stop_timestamp - start_timestamp)
    )
    ...
    #  ##### 2. summary
    start_timestamp = calendar.timegm(time.gmtime())
    significant_genes(prefix, out, cutoff)
    stop_timestamp = calendar.timegm(time.gmtime())
    print(
        "\tTotal time to complete step 7: %d seconds"
        % (stop_timestamp - start_timestamp)
    )
    ...
    print(
        "\t>>>>>Overall time needed from step3 to step7 costs %d seconds"
        % (init_stop_timestamp - init_start_timestamp)
    )
...
if __name__ == "__main__":
    ...
    min_total_cov = sys.argv[5]
    min_single_cov = sys.argv[6]
    prefix = sys.argv[7]
    out = sys.argv[8]
    out = out + prefix + "/"
    ...
    # os.system(cmd)
    model = STAN + modelName + "/" + modelName
    main()
