...
def Intersect_exonicHetSnps(parsed_mpileup,meta_file,prefix,out,min_totalcounts=1,min_singlecounts=0,DEBUG=False,percentile=None):
    altRatio=[]
    tCount=[]
    rCount=[]
    aCount=[]
...
#   chr1    916549  ENSG00000187642  rs6660139
    if DEBUG is True:
        ...
        print("DEBUG USE - finish loading parsed mpileup file: %s\nStarting intersecting with biallelic-het-SNP file"%(parsed_mpileup))
    if len(df): 
        ...
        freq = df6['altCount']/df6['totalCount'] #df6 = pd.concat([df_hetsMeta, df6], axis=1, join="inner")
        ...
        df_overlapped = pd.merge(df6,df_hetsMeta,on=['contig','position'], how="inner") #df6['gene']=df6.apply(lambda x: FindKey(str(x['contig']),str(x['position']),data,if_list=False),axis=1)
        print(df_overlapped.head(n=2))
        df6_med = df_overlapped.groupby(['geneID']).agg({"altRatio":'median'}).reset_index().rename(columns={'altRatio': 'median altRatio'})
        med_atr = np.ndarray.tolist(df6_med['median altRatio'].values)
        df6_nhets = df_overlapped.groupby(by='geneID').agg({'position':pd.Series.nunique}).rename(columns={'position': 'number of hets'})
        
        df6_totalref = df_overlapped.groupby(by='geneID').agg({'refCount':'sum'}).reset_index().rename(columns={'totalRef': 'total refAllele'})
        df6_totalalt = df_overlapped.groupby(by='geneID').agg({'altCount':'sum'}).reset_index().rename(columns={'totalAlt': 'total altAllele'})
        ...
        medAltRatio=med_atr
        title='total alleles >= '+str(min_totalcounts)+', single allele >= '+str(min_singlecounts)
    ...
    print("DEBUG USE - Aggregated pasred mpile up data dim %s saved to %s"%(df_overlapped.size,output1))
    ...
    print("DEBUG USE - Number of genes in this file for lambda prediction is %s saved to %s"%(df_summary_3.size,output2))
    return(df_overlapped,df_summary_3)

def generate_modelCount(prefix,gene_df,out=None,DEBUG=False): # force the gene ID to be the target Gene
