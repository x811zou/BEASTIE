...
def Intersect_exonicHetSnps(
    ...
    if len(df_sub): 
        ...
        df_overlapped['snp_window'] = (df_overlapped['transcript_pos'] - df_overlapped.groupby(['geneID','transcriptID'])['transcript_pos'].transform(np.min)) // int(read_length)
        df_overlapped = df_overlapped.sort_values("totalCount", ascending=False).groupby(['geneID','transcriptID','snp_window']).nth(0)
        df_overlapped.reset_index(inplace=True)
        df_overlapped.to_csv(out2,index=False,sep='\t',header=True)

        df_overlapped=df_overlapped.drop(['snp_window','transcript_pos','transcriptID'],axis=1)
        df_overlapped=df_overlapped.drop_duplicates()
        df_overlapped.sort_values(['chrN','pos','geneID'],ascending=[True,True,True], inplace=True)
        df_overlapped_uni = df_overlapped.groupby(['chr','pos']).first().reset_index()
        df_overlapped_uni.to_csv(hetSNP_intersect_unique,index=False,sep='\t',header=True)
    ...
    logging.info('..... Aggregated pasred mpile up data dim {0} saved to {1}'.format(df_overlapped_uni.size,hetSNP_intersect_unique))
    df_summary_3.to_csv(hetSNP_intersect_unique_forlambda_file,index=False,sep='\t',header=True)
    logging.info('..... Number of genes in this file for lambda prediction is {0} saved to {1}'.format(df_summary_3.size,hetSNP_intersect_unique_forlambda_file))
