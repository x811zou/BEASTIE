...
def update_model_input_lambda_phasing(out,prefix,pred_prob_column):
    ...
    outfile=out+str(prefix)+"_parsed_mpileup_exonicHetSnps_model_input_w_phasingError.txt"
    ...
    phasing_error=pd.read_csv(out+str(prefix)+"_logisticRegression_input_phasingErrorpredicted.tsv", header=0,sep='\t')
...
def significant_genes(prefix,out,cutoff):
    ...
    data_beastie_medtheta=pickle.load(open(modeloutput_dir+"model_theta_med/"+str(prefix)+"_parsed_mpileup_exonicHetSnps_model_input_s-0.5.pickle","rb"))
    data_beastie_maxtail=pickle.load(open(modeloutput_dir+"model_prob/"+str(prefix)+"_parsed_mpileup_exonicHetSnps_model_input_s-0.5.pickle","rb"))
    data_beastie_lambda12=pickle.load(open(modeloutput_dir+"model_prob_sum_lambda1.2/"+str(prefix)+"_parsed_mpileup_exonicHetSnps_model_input_s-0.5.pickle","rb"))
    data_beastie_lambda14=pickle.load(open(modeloutput_dir+"model_prob_sum_lambda1.4/"+str(prefix)+"_parsed_mpileup_exonicHetSnps_model_input_s-0.5.pickle","rb"))
    data_beastie_lambda16=pickle.load(open(modeloutput_dir+"model_prob_sum_lambda1.6/"+str(prefix)+"_parsed_mpileup_exonicHetSnps_model_input_s-0.5.pickle","rb"))
    data_beastie_predictedlambda=pickle.load(open(modeloutput_dir+"model_prob_sum_lambda_predicted/"+str(prefix)+"_parsed_mpileup_exonicHetSnps_model_input_s-0.5_a-0.05.pickle","rb"))
        #
    data_modeloutput=pd.read_csv(model_dir+prefix+'_parsed_mpileup_overlapped_exonicHetSnps_Lambda_0.05_prediced.tsv', header=None,sep='\t')
    ...
    data_modeloutput=data_modeloutput.drop(['BEASTIE_maxtail','BEASTIE_sumtail_lambda_12','BEASTIE_sumtail_lambda_14','BEASTIE_sumtail_lambda_16'],axis=1)
    for index,method in enumerate(columns):
        data=df_sub.iloc[:,index]
        if index==0:
            ncount=data_modeloutput[data_modeloutput['BEASTIE_sumtail_lambda_pred']>0.5].count()[8]
        else:
            ncount=data_modeloutput[data_modeloutput['BEASTIE_sumtail_lambda_pred']>0.5].count()[8]
    ...
    data_modeloutput=data_modeloutput.assign(ASE=data_modeloutput.apply(ASE_judge,axis=1))
