...
def runVariant(
    model, fields, input_file, tmp_output_file, stan_output_file, init_file, sigma
):
    if len(fields) >= 5:
...
in_path = sys.argv[5] + "/"
out_path = (
    "/data/allenlab/scarlett/software/cmdstan/examples/"
    + str(model)
    + "/"
    + sys.argv[6]
    + "/output_pkl/"
...
model = (
    "/data/allenlab/scarlett/software/cmdstan/examples/" + str(model) + "/" + str(model)
)
