...
def count_all_het_sites(sample, vcf_filename, annotation_dir, output_filename):
    ...
    for chromosome in range(1, 23):
        ...
        gene_list = reader.loadGenes(
            os.path.join(annotation_dir, f"chr{chromosome}.gtf")
        )
        ...
        for gene in gene_list:
            ...
            for line in lines:
                ...
                out_file.write(
                    f"{chrom}\t{chromN}\t{geneID}\t{pos}\t{transcriptCoord}\t{rs}\t{genotype}\n"
                )
...
if __name__ == "__main__":
    ...
    output_filename = os.path.join(
        hets_meta_output_dir, "Allchr_hets_all_transcript.tsv"
    )
