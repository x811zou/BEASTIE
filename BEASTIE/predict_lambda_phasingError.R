...
############################################## 1. predict lambda #####################################################
lambda.fit.simulation<- readRDS(paste0("/Users/scarlett/Box/Allen_lab/Backup/BEASTIE3/github_example/LinearReg_",model,"_fitted_model_lambda.rds"),sep="")
predict_lambda_realdata <- function(alpha,local_dir,sample,in_data){
  in_data<-read.delim(file.path(paste0(local_dir,"output/",sample,"_parsed_mpileup_overlapped_exonicHetSnps_forLambda.tsv",sep="")),header=TRUE,sep="\t")
  out_data<-paste0(local_dir,"output/",prefix,"_parsed_mpileup_overlapped_exonicHetSnps_","Lambda_",as.character(as.numeric(alpha)),"_prediced.tsv",sep="")  
...
sample_info<-read.delim(file.path(paste0(local_dir,"output/",prefix,"_parsed_mpileup_overlapped_exonicHetSnps.tsv",sep="")),header=TRUE,sep="\t")
...
# -1 for those NAs
out_data=paste0(local_dir,"output",prefix,"_logisticRegression_input_phasingErrorpredicted.tsv",sep="")
