...
predict_lambda_realdata <- function(alpha,local_dir,sample,in_data){
  ...
  out_data<-paste0(local_dir,"output/",sample,"_parsed_mpileup_overlapped_exonicHetSnps_","Lambda_",as.character(as.numeric(alpha)),"_prediced.tsv",sep="")  
...
sample_info<-read.delim(file.path(paste0(local_dir,"output/",sample,"_parsed_mpileup_overlapped_exonicHetSnps.tsv",sep="")),header=TRUE,sep="\t")
...
# -1 for those NAs
out_data=paste0(local_dir,"output",sample,"_logisticRegression_input_phasingErrorpredicted.tsv",sep="")
