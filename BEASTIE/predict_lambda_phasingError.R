...
############################################## 1. predict lambda #####################################################
lambda.fit.simulation<- readRDS("/Users/scarlett/Box/Allen_lab/Backup/BEASTIE3/github_example/LinearReg_iBEASTIE_fitted_model_lambda.rds")
predict_lambda_realdata <- function(alpha,local_dir,sample,in_data){
  ...
  out_data<-paste0(local_dir,"output/NA12878_parsed_mpileup_overlapped_exonicHetSnps_","Lambda_",as.character(as.numeric(alpha)),"_prediced.tsv",sep="")  
...
sample_info<-read.delim(file.path(paste0(local_dir,"output/NA12878_parsed_mpileup_overlapped_exonicHetSnps.tsv",sep="")),header=TRUE,sep="\t")
...
# -1 for those NAs
out_data=paste0(local_dir,"output/NA12878_logisticRegression_input_phasingErrorpredicted.tsv",sep="")
