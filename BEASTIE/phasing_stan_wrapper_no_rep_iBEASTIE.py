...
def runVariant_predictLambda_simulation(
    model,
    fields,
    tmp_output_file,
    stan_output_file,
    ...
    if len(fields) >= 4:
        ...
        # writeInputsFile(fields,tmp_output_file)
        writeInitializationFile(init_file)
        cmd = "%s sample data file=%s init=%s output file=%s" % (
            model,
            tmp_output_file,
            init_file,
            stan_output_file,
        ...
        (
            ...
            prob_lambda3,
            sum_lambda3,
        ) = parse_lambda_validation(thetas, alphas, lambdas_file, 1)
        (
            prob_lambda11,
            sum_lambda11,
            prob_lambda22,
            ...
            prob_lambda33,
            sum_lambda33,
        ) = parse_lambda_validation(thetas, alphas, lambdas_file, 2)
        return (
            prob_lambda1,
            sum_lambda1,
            prob_lambda2,
...
def parse_stan_output(
    ...
    alpha_list,
    lambdas_file=None,
    predicted_lambda_modeldata=False,
    ...
    if str(default_lambda) == "True":
        ...
        if not os.path.exists(out_path + "model_theta/"):
            os.makedirs(out_path + "model_theta/")
        if not os.path.exists(out_path + "model_theta_med/"):
            os.makedirs(out_path + "model_theta_med/")
        if not os.path.exists(out_path + "model_prob/"):
            os.makedirs(out_path + "model_prob/")
        if not os.path.exists(out_path + "model_prob_tail_lambda1.2/"):
            os.makedirs(out_path + "model_prob_tail_lambda1.2/")
        if not os.path.exists(out_path + "model_prob_tail_lambda1.3/"):
            os.makedirs(out_path + "model_prob_tail_lambda1.3/")
        if not os.path.exists(out_path + "model_prob_tail_lambda1.4/"):
            os.makedirs(out_path + "model_prob_tail_lambda1.4/")
        if not os.path.exists(out_path + "model_prob_tail_lambda1.5/"):
            os.makedirs(out_path + "model_prob_tail_lambda1.5/")
        if not os.path.exists(out_path + "model_prob_tail_lambda1.6/"):
            os.makedirs(out_path + "model_prob_tail_lambda1.6/")
        if not os.path.exists(out_path + "model_prob_tail_lambda1.8/"):
            os.makedirs(out_path + "model_prob_tail_lambda1.8/")
        if not os.path.exists(out_path + "model_prob_sum_lambda1.2/"):
            os.makedirs(out_path + "model_prob_sum_lambda1.2/")
        if not os.path.exists(out_path + "model_prob_sum_lambda1.3/"):
            os.makedirs(out_path + "model_prob_sum_lambda1.3/")
        if not os.path.exists(out_path + "model_prob_sum_lambda1.4/"):
            os.makedirs(out_path + "model_prob_sum_lambda1.4/")
        if not os.path.exists(out_path + "model_prob_sum_lambda1.5/"):
            os.makedirs(out_path + "model_prob_sum_lambda1.5/")
        if not os.path.exists(out_path + "model_prob_sum_lambda1.6/"):
            os.makedirs(out_path + "model_prob_sum_lambda1.6/")
        if not os.path.exists(out_path + "model_prob_sum_lambda1.8/"):
            os.makedirs(out_path + "model_prob_sum_lambda1.8/")
        out1 = out_path + "model_theta/" + str(outfix) + "_s-" + str(sigma) + ".pickle"
        out2 = (
            out_path + "model_theta_med/" + str(outfix) + "_s-" + str(sigma) + ".pickle"
        )
        out3 = out_path + "model_prob/" + str(outfix) + "_s-" + str(sigma) + ".pickle"
        out4 = (
            out_path
            + "model_prob_tail_lambda1.2/"
            + str(outfix)
            + "_s-"
            + str(sigma)
            + ".pickle"
        ...
        with open(input_file, "rt") as IN:
            ...
            for line in IN:
                ...
                (
                    ...
                    prob_lambda33,
                    sum_lambda33,
                ) = runVariant_predictLambda_simulation(
                    models,
                    fields,
                    tmp_output_file,
                    stan_output_file,
...
def main():
    ...
    parse_stan_output(
        ...
        alphas,
        selfdefined_alpha,
        lambdas_file=lambdas_file,
        predicted_lambda_modeldata=predicted_lambda_modeldata,
        default_lambda=default_lambda,
        selfdefined_lambda=selfdefined_lambda,
    )
